...
# UCL: Added Octopus 15 and 16, misc fixes.
...
class Octopus(CMakePackage, AutotoolsPackage, CudaPackage):
    ...
    version(
        "16.3",
        sha256="def2b8f67d7d464a7da26226c724f62c16592f1836a98757079a2b3d822e2494",
        url="https://gitlab.com/octopus-code/octopus/-/archive/16.3/octopus-16.3.tar.gz",
    )
    version(
        "16.2",
    ...
    depends_on("libxc@5:5.2.3", when="@10:12")
    depends_on("libxc@5.1.0:6.2.2", when="@14:16")
...
class CMakeBuilder(cmake.CMakeBuilder):
    ...
    def cmake_args(self):
        ...
        args = [
            ...
            self.invert_bool_variant("CMAKE_DISABLE_FIND_PACKAGE_BerkeleyGW", "berkeleygw"),
        ...
        if "^fftw" in self.spec:
            ...
            raise InstallError(f"Unsupported provider for fftw-api\n{self.spec}")
        return args
...
class AutotoolsBuilder(autotools.AutotoolsBuilder):
    def configure_args(self):
        ...
        if "+mpi" in spec:  # we build with MPI
            args.extend(
                ["--enable-mpi", "CC=%s" % spec["mpi"].mpicc, "FC=%s" % spec["mpi"].mpifc]
            )
        else:
            args.extend(["CC=%s" % self.compiler.cc, "FC=%s" % self.compiler.fc])
        ...
        if "+sparskit" in spec:
            args.append(
                "--with-sparskit=%s" % os.path.join(spec["sparskit"].prefix.lib, "libskit.a")
            )
        if "+etsf-io" in spec:
            args.append("--with-etsf-io-prefix=%s" % spec["etsf-io"].prefix)
        # --with-pfft-prefix=${prefix} --with-mpifftw-prefix=${prefix}
